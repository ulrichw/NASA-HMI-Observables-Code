...
char *module_name    = "lookup_Iripple";      //name of the module
...
ModuleArgs_t module_args[] =        
{
     ...
     {ARG_DOUBLE ,"initialNB", "","initial phase of NB"},
     {ARG_DOUBLE ,"initialWB", "", "initial phase of WB"},
     {ARG_DOUBLE ,"initialE1", "", "initial phase of E1"},
     {ARG_DOUBLE ,"K1","","K1"},
     {ARG_DOUBLE ,"K2","","K2"},
     {ARG_DOUBLE ,"K3","","K3"},
     {ARG_DOUBLE ,"K4","","K4"},
     {ARG_DOUBLE ,"K5","","K5"},
     {ARG_DOUBLE ,"K6","","K6"},
     //{ARG_DOUBLE,  "width","","width of the Gaussian part of the Voigt profile"},
     //{ARG_DOUBLE,  "depth","","related to depth of Voigt profile"},
     //{ARG_DOUBLE,  "a","","damping of Voigt profile"},
     ...
     {ARG_INT    , "line","0", "line profile used (0=Voigt,1=R. Ulrich"},
     {ARG_END}
};
...
/* y is the damping parameter                                                          */
/* x is (\lambda-\lambda_0)/width                                                      */
/*                                                                                     */
/***************************************************************************************/
...
int DoIt(void) {
  ...
  double initialNB= cmdparams_get_double(&cmdparams,"initialNB",NULL);
  initialNB=initialNB*M_PI/180.0;
  double initialWB= cmdparams_get_double(&cmdparams,"initialWB",NULL);
  initialWB=initialWB*M_PI/180.0;
  double initialE1= cmdparams_get_double(&cmdparams,"initialE1",NULL);
  initialE1=initialE1*M_PI/180.0;
  double initK1   = cmdparams_get_double(&cmdparams,"K1",NULL);
  double initK2   = cmdparams_get_double(&cmdparams,"K2",NULL);
  double initK3   = cmdparams_get_double(&cmdparams,"K3",NULL);
  double initK4   = cmdparams_get_double(&cmdparams,"K4",NULL);
  double initK5   = cmdparams_get_double(&cmdparams,"K5",NULL);
  double initK6   = cmdparams_get_double(&cmdparams,"K6",NULL);
  //double fdepthg  = cmdparams_get_float(&cmdparams,"depth",NULL);//0.5625;//0.63;//0.56;//0.642710;//0.651742;
  //double fwidthg  = cmdparams_get_float(&cmdparams,"width",NULL);//0.06415;//0.069334;//0.071124;//0.0687;//0.069334;//0.069598;
  //double a        = cmdparams_get_float(&cmdparams,"a",NULL);
  ...
  int lineparam   = cmdparams_get_int(&cmdparams,   "line"  , NULL); 

  char COMMENT[]="Fe I line profile is a Voigt profile."; //comment about the looku-p table code
  ...
  //wavelength grid
  int nlam      = 32001;                                 //the wavelength range must be, at least, [-8,+8] A around the target wavelength. That captures 99.76% of the filters integral values. MUST BE AN ODD NUMBER BECAUSE WE WANT THE ZERO
  ...
  double FWHMCoeffs[2]={221.34559,-45.521771};            //FWHM of the solar line in milliAngstrom (result from a Gaussian fit in the range [-0.055,0.055] Angstroms)
  ...
  float fdepthg;
  float fwidthg;
  float templineref2[referencenlam];  //reference solar line: solar line of R. Ulrich at disk center, zero velocity, interpolated on a fine grid, and centered
  float wavelengthref2[referencenlam];//wavelength grid for the reference wavelength
  double wavelength2[referencenlam];
  double lineprofile2[referencenlam];
  double lineprofile[nlam];
  double linetemp[referencenlam];
  int ij;
  double minimum1=0.0;
  double minimum0=0.65384230; //linedepth, i.e. 1-minimum
  double FWHM0=0.093013103;
  ...
  if(calibration == 3) //TEST CALIBRATION
    {
      Icg=1.0;
      fdepthg=0.62;
      fwidthg=0.057;
  ...
  /*for(k=0;k<referencenlam;++k) 
    {
      ...
      templineref[k]= Icg-fdepthg*humlik(l,a);
      }*/


  //to read the line in idl:
  //d=read_binary("/home/jsoc/cvs/Development/JSOC/proj/lev1.5_hmi/apps/ReferenceFeLine.bin",DATA_TYPE=4)
  //w=read_binary("/home/jsoc/cvs/Development/JSOC/proj/lev1.5_hmi/apps/ReferenceWavelength.bin",DATA_TYPE=4)
  if(lineparam == 1){
    printf("Solar line profile file used = %s\n",referencesolarline);
    fp = fopen(referencesolarline,"rb");
    fread(templineref2,sizeof(float),referencenlam,fp);
    fclose(fp);
    fp = fopen(referencewavelength,"rb");
    fread(wavelengthref2,sizeof(float),referencenlam,fp);
    fclose(fp);
 
    fwidthg=0.108;//0.111215;
    fdepthg=0.642009/1.530551;
    Icg=1.0;
       
    //change the linewidth and linedepth
    for(k=0;k<referencenlam;++k)
      {
	linetemp[k] = (1.0-templineref2[k])*fdepthg/minimum0; //scaling by the ratio of linedepths
	linetemp[k] =  1.0-linetemp[k];
	wavelength2[k]= wavelengthref2[k]*fwidthg/FWHM0;
      }
    for (ij=0; ij<referencenlam; ij++)
      {
	if((wavelengthref[ij] < wavelength2[0]) || (wavelengthref[ij] > wavelength2[referencenlam-1])) templineref[ij] = Icg;
	else
	  {   
	    for(jj=1; jj<referencenlam; jj++)
	      {      
		if(wavelengthref[ij]<=wavelength2[jj])
		  {		   
		    templineref[ij] = (wavelengthref[ij]-wavelength2[jj-1]) / (wavelength2[jj]-wavelength2[jj-1]) * (linetemp[jj]-linetemp[jj-1]) + linetemp[jj-1];
		    templineref[ij] = templineref[ij]*Icg;
		      break;
		  }
	      }
	  }
	...
	for(k=0;k<referencenlam;++k) 
	  {
	    l=wavelengthref[k]/fwidthg;
	    if(fabs(l) <= 26.5) templineref[k]= Icg-fdepthg*exp(-l*l)*(1.0-a*2.0/sqrt(M_PI)*(1.0/2.0/l/l)*((4.0*l*l+3.0)*(l*l+1.0)*exp(-l*l)-1.0/l/l*(2.0*l*l+3.0)*sinh(l*l)))+0.005*exp(-(wavelengthref[k]+0.2)*(wavelengthref[k]+0.2)/0.13/0.13);
	    else templineref[k]=Icg+0.0075*exp(-(wavelengthref[k]+0.2)*(wavelengthref[k]+0.2)/0.13/0.13);//+0.005*exp(-(wavelengthref[k]+0.2)*(wavelengthref[k]+0.2)/0.13/0.13);
	  }
      }
  }
  //find the minimum of the line
  ...
  pv1 = dvtune*(double)(N-1); //NB: NOT CORRECT, SHOULD BE MULTIPLIED BY N
  pv2 = pv1/2.;
...
#pragma omp parallel for default(none) private(location,iii,i,j,k,values,lineprofile,shiftlam,inten,filters,f1c,f1s,vel1,f2c,f2s,vel2,row,column,FWHM,minimum,wavelength2,lineprofile2,lyot,phaseE2,phaseE3,phaseE4,phaseE5,contrastE2,contrastE3,contrastE4,contrastE5,contrastNB,contrastWB,contrastE1,phaseNB,phaseWB,phaseE1,IrippleNB,IrippleWB,IrippleE1) shared(FSR,blockerint,cosi,sini,cos2i,sin2i,distance,nx2,ny2,ntest,nlam,lam,vtest,dlam,dlamdv,wavelength,N,cmich,phases,pv1,pv2,vel,axisout,nelement,ydefault,minimumCoeffs,FWHMCoeffs,WRONGDISTANCE,BUFFERDISTANCE,HCME1phase,HCMWBphase,HCMNBphase,phaseNT,contrastNT,contrastT,maxshiftlam,wavelengthref,referencenlam,solarradiusmax,NOMINALSCALE,templineref,initK1,initK2,initK3,initK4,initK5,initK6,initialE1,initialWB,initialNB)
  for(iii=0;iii<nx2*ny2;++iii)
      ...
      if(distance[iii] != WRONGDISTANCE)
	{
	  ...
	  contrastE2=(double)contrastNT[location];//-0.984970+0.981352;//WARNING, USED THE RESULT OF phasemaps_test_voigt_Iripple2_all
	  ...
	  phaseE2   =(double)phaseNT[location];//+(6.340168-8.9319430)*M_PI/180.; //WARNING: USING RESULT OF phasemaps_test_voigt_Iripple2_all
	  ...
	  for(i=0;i<N;i++) 
	    {
	      ...
	      //calculate I-ripple
	      IrippleE1       = ( 1.0 + (initK1*cos((-HCME1phase[i]+initialE1)/2.0)+initK2*sin((-HCME1phase[i]+initialE1)/2.0))*(initK1*cos((-HCME1phase[i]+initialE1)/2.0)+initK2*sin((-HCME1phase[i]+initialE1)/2.0)));
	      IrippleWB       = ( 1.0 + (initK3*cos((HCMWBphase[i]+initialWB)/2.0) +initK4*sin((HCMWBphase[i]+initialWB)/2.0)) *(initK3*cos((HCMWBphase[i]+initialWB)/2.0)+initK4*sin((HCMWBphase[i]+initialWB)/2.0))  );
	      IrippleNB       = ( 1.0 + (initK5*cos((HCMNBphase[i]+initialNB)/2.0) +initK6*sin((HCMNBphase[i]+initialNB)/2.0)) *(initK5*cos((HCMNBphase[i]+initialNB)/2.0)+initK6*sin((HCMNBphase[i]+initialNB)/2.0))  );
	      
	      for(j=0;j<nlam;j++) filters[i][j] = lyot[j]*(1.+contrastNB*cos(cmich[0]*lam[j]+HCMNBphase[i]+phaseNB))/2.*(1.+contrastWB*cos(cmich[1]*lam[j]+HCMWBphase[i]+phaseWB))/2.*(1.+contrastE1*cos(cmich[2]*lam[j]-HCME1phase[i]+phaseE1))/2.*IrippleE1*IrippleWB*IrippleNB;
	    }
	  ...
	  if(row == 64 && column == 64)
	    {
	      ...
	      for(j=0;j<nlam;++j) fprintf(fp,"%f %f %f %f %f %f %f %f\n",lam[j],filters[0][j],filters[1][j],filters[2][j],filters[3][j],filters[4][j],filters[5][j],lineprofile[j]);
	      	  for(i=0;i<N;i++) 
		    {
		      ...
		      IrippleWB       = ( 1.0 + (initK3*cos((HCMWBphase[i]+initialWB)/2.0) +initK4*sin((HCMWBphase[i]+initialWB)/2.0)) *(initK3*cos((HCMWBphase[i]+initialWB)/2.0)+initK4*sin((HCMWBphase[i]+initialWB)/2.0))  );
		      IrippleNB       = ( 1.0 + (initK5*cos((HCMNBphase[i]+initialNB)/2.0) +initK6*sin((HCMNBphase[i]+initialNB)/2.0)) *(initK5*cos((HCMNBphase[i]+initialNB)/2.0)+initK6*sin((HCMNBphase[i]+initialNB)/2.0))  );
		      printf("PV1 = %f %f %f %f\n",pv1,IrippleNB,IrippleWB,IrippleE1);
			     
		     }
		      
		  printf("PHASES: %f %f %f %f %f %f %f\n",phaseNB,phaseWB,phaseE1,phaseE2,phaseE3,phaseE4,phaseE5);
		  printf("CONTRASTS: %f %f %f %f %f %f %f\n",contrastNB,contrastWB,contrastE1,contrastE2,contrastE3,contrastE4,contrastE5);
  ...
  if (status == DRMS_SUCCESS)
    {	  
      ...
      //HMI filter profiles parameters
      ...
      //set Fe I line profile
      status = drms_setkey_float(recout,"a",a);             //damping of Voigt profile
      status = drms_setkey_float(recout,"depth",fdepthg);   //depth of Voigt profile
      status = drms_setkey_float(recout,"width",fwidthg);   //width of Voigt profile


      //set I-ripple keywords
      status = drms_setkey_float(recout,"K1",initK1);
      status = drms_setkey_float(recout,"K2",initK2);
      status = drms_setkey_float(recout,"K3",initK3);
      status = drms_setkey_float(recout,"K4",initK4);
      status = drms_setkey_float(recout,"K5",initK5);
      status = drms_setkey_float(recout,"K6",initK6);
      status = drms_setkey_float(recout,"initialE1",initialE1);
      status = drms_setkey_float(recout,"initialWB",initialWB);
      status = drms_setkey_float(recout,"initialNB",initialNB);

      //set the keywords for the HCM positions
